...
app.MapPost("/hashtags", async (HashtagRequest payload) =>
{
	...
	while (allHashtags.Count < count && retries < MAX_RETRIES)
	{
		...
		var prompt = $"Generate a list of 2 hashtags for the given text. Respond using JSON (array of strings named hashtags). Text: {payload.text}.";
		var requestBody = new
		{
			model = model,
			prompt = prompt,
	...
	Console.WriteLine($"[Ollama raw response] {ollamaResponse}");
		...
		Console.WriteLine($"[Filtered hashtags count] {filteredHashtags.Length}, [Total unique hashtags so far] {allHashtags.Count + filteredHashtags.Count(h => !allHashtags.Contains(h))}");
	...
	if (allHashtags.Count < count)
	{
		return Results.Ok(new { count, model, hashtags = allHashtags, error = error ?? $"Could not generate the requested number of hashtags after {MAX_RETRIES} attempts." });
	}
